...
skew_true <- array(seq(0.2, 2.4, length.out = 24), dim = c(3,4,2))
...
```{r}
invgauss_draws <- rtinvgauss(n = 1e3, mu = mu_true,
                             skew = skew_true, sigmas = list(S1, S2, S3))
...
est_mean_draws <- invgauss_est$mu + 1/invgauss_est$kappa * invgauss_est$skew

true_mean_draws <- mu_true + 1/2 * skew_true

frob_norm(est_mean_draws - true_mean_draws) / frob_norm(true_mean_draws)

frob_norm(invgauss_est$mu - mu_true) / frob_norm(mu_true)
frob_norm(invgauss_est$skew - skew_true) / frob_norm(skew_true)
