...
```{r}
genhyper_draws <- rtgenhyper(n = 1e3, mu = rand_mu)
```


```{r}
em_est_genhyper(genhyper_draws)
```


## Tensor variate variance gamma
```{r}
vargamma_draws <- rtvargamma(n = 1e3, mu = rand_mu)
```



```{r}
vargam_est <- em_est_vargamma(vargamma_draws)

vargam_est$mu 
vargam_est$skew 
vargam_est$sigmas |> lapply(round, 3)
```
...
```{r}
invgauss_est <- em_est_invgauss(vargamma_draws)

invgauss_est$mu
invgauss_est$skew |> lapply(round, 3)
