...
cifar_tree_indices <- which(cifar100$y == 48)

n <- length(cifar_tree_indices)

imgs <- vector("list", n)

for (i in seq_len(n)) {
  imgs[[i]] <- cifar100[[cifar_tree_indices[i]]]$x
}

imgs <- imgs[1:200]

res <- model_compare(imgs)

res_normal <- tensor_mle(imgs, model = "normal", quiet = FALSE)
res_skewt <- tensor_mle(imgs, model = "skewt", quiet = FALSE)
res_vargam <- tensor_mle(imgs, model = "vargamma", quiet = FALSE)
res_invgauss <- tensor_mle(imgs, model = "invgauss", quiet = FALSE)
res_genhyper <- tensor_mle(imgs, model = "genhyper", quiet = FALSE)
...
```{r}
tic()
res <- tensor_mle(imgs, model = "normal", quiet = FALSE)
toc()
```


```{r}
plot_matrix(res$mu[, , 1]) +
  plot_matrix(res$mu[, , 2]) +
  plot_matrix(res$mu[, , 3]) 
```
