...
#   ggplot() + 
#   geom_point(aes(x = logliks, y = row)) +
#   theme_minimal()
# 
# 
# tibble(converge) |> 
#   mutate(row = row_number()) |> 
#   ggplot() + 
#   geom_point(aes(x = row, y = converge)) +
#   theme_minimal()
# 
# 
# converge <- rep(0, 100)
...
```{r}
fit_mean <- invgauss_est$mu + invgauss_est$skew / invgauss_est$kappa

true_mean <- mu_true + skew_true / kappa_true

frob_norm_diff(fit_mean, true_mean)

frob_norm_diff(invgauss_est$skew / invgauss_est$kappa,
               skew_true / kappa_true)

mean(invgauss_est$Ew)
```
...
skew_true <- array(rnorm(24), dim = c(3,4,2))
kappa_true <- 2
...
# profvis::profvis({
#   fit <- tensor_mle(data, max_iter = 100, model = "invgauss",
#                      tol = 1e-6, quiet = TRUE)
# })
#250 seconds
```
...
```{r}
vargamma_draws <- rtvargamma(n = 1e3, mu = mu_true, sigmas = list(S1, S2, S3))
```
...
frob_norm_diff(vargamma_est$skew, array(1, dim = c(3, 4, 2)))
