...
skewt_density <-
  ...
  mutate(density = dtskewt(c(x11, x21), 
                           mu = c(1, 2), sigmas = sigmas, skew = c(2, 1), 
                           nu = 20))
...
eval_points <- expand_grid(x11 = seq(-4, 4, length.out = 150),
                           x21 = seq(-4, 4, length.out = 150))

norm_density <-
  eval_points |> 
  rowwise() |> 
  mutate(density = dtnorm(c(x11, x21), mu = mu, sigmas = sigmas)) |> 
  ungroup()

ggplot(norm_density, aes(x = x11, y = x21, fill = density)) +
  geom_raster() +
  scale_fill_viridis_c() +
  coord_fixed() +
...
matrix_skewt_density <-
  eval_points |> 
  rowwise() |> 
  mutate(log_density = dtskewt(matrix(c(x11, x21, x12, x22), nrow = 2),
                               mu = mu, sigmas = list(s1, s2), skew = skew,
                               nu = 20, log = TRUE)) |>
  ungroup()
...
matrix_normal_density <-
  eval_points |> 
  rowwise() |> 
  mutate(log_density = dtnorm(matrix(c(x11, x21, x12, x22), nrow = 2),
                              mu = mu, sigmas = list(s1, s2), log = TRUE)) |>
  ungroup()
