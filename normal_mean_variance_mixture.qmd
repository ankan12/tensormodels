...
optim_mles <- fit_beta_lambda(x_missing, y)
```
...
em_mles <- fit_beta_lambda_em(x_missing, y)
```


```{r}
tibble(true_lam = lambdas, optim_lam = optim_mles$lambda) |> 
  ggplot() + 
  geom_point(aes(x = lambdas, y = optim_lam)) +
  geom_abline(color = "red", lty = 2) +
  labs(x = "True Lambdas", y = "Optim Lambdas") +
  theme_minimal() +
tibble(true_lam = lambdas, em_lam = em_mles$lambda) |> 
  ggplot() + 
  geom_point(aes(x = lambdas, y = em_lam)) +
  geom_abline(color = "red", lty = 2) +
  labs(x = "True Lambdas", y = "EM Lambdas") +
  theme_minimal() +
tibble(true_lam = lambdas, mle_lam = lambdas_hat) |> 
  ggplot() + 
  geom_point(aes(x = lambdas, y = mle_lam)) +
  geom_abline(color = "red", lty = 2) +
  labs(x = "True Lambdas", y = "MLE Lambdas") +
  theme_minimal()     


tibble(true_lam = lambdas, mle_lam = lambdas_hat,
       optim_lam = optim_mles$lambda, em_lam = em_mles$lambda) |> 
  mutate(id_lam = row_number()) |> 
  pivot_longer(cols = true_lam:em_lam) |>   
  mutate(id_lam = factor(id_lam), name = factor(name)) |>
  ggplot() +
  geom_point(aes(x = id_lam, y = value, color = name)) +
  facet_grid(~name) +
  theme_minimal()
```
