...
em_est <- function(draws, max_iter = 1000, tol = 1e-6, quiet = TRUE, model) {
  if (!model %in% c("skewt", "vargamma", "invgamma", "genhyper")) {
    stop("Not a valid model. Must be skewt, vargamma, invgamma, or genhyper")
  }
  ...
  for (t in 1:max_iter) {
    ...
    switch(
      model,
      "skewt" = mean_change <- mean_rel_change(
        new_mu,
        mu,
        new_skew,
        skew,
      ...
      "invgamma" = mean_change <- mean_rel_change(
        new_mu,
        mu,
        new_skew,
        skew,
